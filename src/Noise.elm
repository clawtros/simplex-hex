...
getFloat : Array Float -> Int -> Float
getFloat arr i =
    Array.get i arr
...
getCornerOffset3d : Float -> Float -> Float -> Offset
    ...
    if t < 0 then
        ...
        let
            ...
            t_ =
                t * t
        in
        t_ * t_ * (getFloat grad3 gi * x + getFloat grad3 (gi + 1) * y)
    ...
    if t < 0 then
        ...
        let
            ...
            t_ =
                t * t
        in
        t_ * t_ * (getFloat grad3 gi * x + getFloat grad3 (gi + 1) * y + getFloat grad3 (gi + 2) * z)
...
getN4d x y z w i j k l perm permMod12 =
    ...
    if t < 0 then
        ...
        let
            gi =
                remainderBy (32 * 4) (get perm i + (get perm j + (get perm k + get perm l)))

            t_ =
                t * t
        in
        t_ * t_ * (getFloat grad4 gi * x + getFloat grad4 (gi + 1) * y + getFloat grad4 (gi + 2) * z + getFloat grad4 (gi + 3) * w)
...
noise3d { perm, permMod12 } xin yin zin =
    let
        ...
        z0 =
            zin - z0_
    in
    case getCornerOffset3d x0 y0 z0 of
        Offset i1 j1 k1 i2 j2 k2 ->
            let
                x1 =
                    x0 - toFloat i1 + g3
